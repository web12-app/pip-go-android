...
function useMemo(factory, deps) {
  const inst = current;
  const i = inst.cursor++;
  const box = i < inst.slots.length ? inst.slots[i] : void 0;
  if (box && depsEqual(box.deps, deps)) return box.value;
  const value = factory();
...
function useEffect(effect, deps) {
  const inst = current;
  const i = inst.cursor;
  const slotIdx = inst.cursor;
  const existing = slotIdx < inst.slots.length ? inst.slots[slotIdx] : void 0;
  if (!existing || !depsEqual(existing.deps, deps ?? null)) {
    inst.slots[slotIdx] = { deps: deps ?? null, cleanup: existing?.cleanup };
    inst.pendingEffects.push(() => {
      const rec = inst.slots[slotIdx];
      if (rec.cleanup && typeof rec.cleanup === "function") {
        try {
          rec.cleanup();
        } catch (e) {
          console.error("[pipgo] effect cleanup failed", e);
        }
      }
      rec.cleanup = effect();
      rec.deps = deps ?? null;
...
async function http(method, url, body, init) {
  if (init?.signal?.aborted) return { ok: false, status: 0, data: null, error: "aborted" };
  const callId = nextCallId();
  const p = new Promise((resolve, reject) => {
    pending.set(callId, { resolve: (v) => resolve(v), reject });
    if (init?.signal) init.signal.addEventListener("abort", () => {
      if (pending.has(callId)) {
        pending.delete(callId);
        post({ op: "http-cancel", callId });
  ...
  post({
    ...
    timeoutMs: init?.timeoutMs ?? 15e3
  });
  return p;
}
var api = {
  get: (url, init) => http("GET", url, void 0, init),
  post: (url, body, init) => http("POST", url, body, init),
  put: (url, body, init) => http("PUT", url, body, init),
  delete: (url, init) => http("DELETE", url, void 0, init)
};
var timerKinds = /* @__PURE__ */ new Map();
function setTimeout(fn, ms) {
  const callId = nextCallId("t");
  timerKinds.set(callId, "timeout");
  pending.set(callId, { resolve: () => {
    timerKinds.delete(callId);
    try {
      fn();
    } catch (e) {
      console.error("[pipgo] timer error", e);
    }
  }, reject: () => {
  } });
  post({ op: "timer", callId, kind: "timeout", delayMs: Math.max(0, ms | 0) });
  return callId;
}
function setInterval(fn, ms) {
  const callId = nextCallId("t");
  timerKinds.set(callId, "interval");
  pending.set(callId, { resolve: () => {
    try {
      fn();
    } catch (e) {
  ...
  post({ op: "timer", callId, kind: "interval", delayMs: Math.max(1, ms | 0) });
  return callId;
}
function clearTimeout(id2) {
  timerKinds.delete(id2);
  post({ op: "timer-clear", callId: id2 });
  pending.delete(id2);
}
...
var Storage = {
  set(key, value) {
    return globalThis.__PIP_GO_HOST__.storageSet(key, JSON.stringify(value ?? null));
  },
  get(key) {
    const raw = globalThis.__PIP_GO_HOST__.storageGet(key);
  ...
  remove(key) {
    return globalThis.__PIP_GO_HOST__.storageRemove(key);
  }
};
function getDeviceInfo() {
  const raw = globalThis.__PIP_GO_HOST__.deviceInfoJson();
  ...
  return {
    ...
    height: d.height ?? 0,
    density: d.density ?? 1,
    safeArea: d.safeArea ?? { top: 0, bottom: 0, left: 0, right: 0 }
  };
}
var screenStack = [];
...
// src/index.ts
var Device = { getInfo: getDeviceInfo };
var __jsxRuntime = jsx_exports;
export {
  App,
  Button,
  Card,
